 ...
 */
function get_alphabet(str: string): string[] {
 ...
 */
function mtf(str: string, Sigma: string[] = null): number[] {
    if (Sigma === null)
 ...
 */
function gamma(i: number): string {
 ...
 */
function inv_gamma(bitstream: string): number[] {
    const output: number[] = [];
    let j = 0;
 ...
 * @returns The encoded string.
 */
function delta(i: number): string {
 ...
 * @returns The decoded numbers.
 */
function inv_delta(bitstream: string): number[] {
    const output: number[] = [];
    let L = 0;
 ...
 */
function lyndon(str: string): number[] {
    const fact: number[] = [-1];
    let k = -1;
...
/**
 * A Segment is represented by a start index, and end index, and a restart index. It is intended
 * to define significant indices in cyclic rotations of contiguous chunks of a given string.
 * 
 * For example, consider the string `Hello, world!` and the Segment
 * {
 *   start: 3,
 *   end: 5,
 *   restart: 2,
 * }
 * 
 * This represents the sequence of characters ['l', 'o', ',', 'l'], which is a cyclic rotation of
 * the underlying chunk starting at index 2 and ending at index 5, `llo,`.
 */
interface Segment {
    /** The index of the first character in the Segment. */
    start: number;

    /** The index of the last character in the underlying chunk. */
    end: number;

    /** The index of the first character in the underlying chunk. */
    restart: number;
}
 ...
 */
function get_segment_rotations(segment: Segment): Segment[] {
    const rots: Segment[] = [];
 ...
 */
function get_periodic_comparator(str: string): (i: Segment, j: Segment) => number {
    return (i, j) => {
 ...
 */
function get_char_map(Sigma: string[]): Map<string, number> {
 ...
 */
function match(str: string, Sigma: string[]): number[] {
    const theta: number[] = [];
    const char_map = get_char_map(Sigma);
    const counts: number[] = Array(Sigma.length).fill(0);
    for (let i = 0; i < str.length; ++i)
        counts[char_map[str[i]]] += 1;
    const before: number[] = Array(Sigma.length).fill(0);
    for (let i = 1; i < Sigma.length; ++i)
        before[i] = before[i - 1] + counts[i - 1];
    const seen: number[] = Array(Sigma.length).fill(0);
    for (let i = 0; i < str.length; ++i) {
 ...
 */
function multithread(str: string, theta: number[]): string {
    const T = theta.slice();
    const alpha: string[] = [];
 ...
 */
function inv_bwt(str: string): string {
 ...
 */
function inv_mtf(bitstream: string, Sigma: string[]): string {
    ...
    $(".nav-tabs > li").on("click", function() {
        if (modified) {
            if (selection === "#input") {
                ...
                alphabet_str = <string>$("#bwt-mtf-Sigma").val();
                bwt_mtf = <string>$("#bwt-mtf").val();
